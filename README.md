...
출력 언어 (D18):
- **에러 메시지와 로그 파일은 항상 영어**입니다 (단일 정본 정책: grep과 에러 매칭이 언어에 흔들리지 않도록).
- stdout 안내·요약 메시지만 언어 선택이 가능합니다: 최상위 `lang: 'ko'` 키(기본 en) 또는 환경변수 `ETL_LANG=ko`(설정보다 우선). `--schedule`이 만드는 cron 라인에는 `ETL_LANG=en`이 박혀 있어 cron.log는 설정과 무관하게 항상 영어입니다.
- help는 별도로 `etl-worker help --lang=ko`로 봅니다 (플래그 > ETL_LANG > en).
...
- **잠자기 한계**: Mac이 잠자면 그 시각의 cron은 건너뜁니다. 워터마크 증분이라 다음 실행이 밀린 변경분을 따라잡으므로 기능상 문제는 없습니다(docs/PRD.md §7). 스케줄러는 이식성을 위해 cron 유지로 확정했으며, launchd(macOS 전용)는 cron을 쓸 수 없는 배치에서만 대안으로 고려합니다.
