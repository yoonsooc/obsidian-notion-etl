...
작성일: 2026-08-20
최종 변경일: 2026-09-03
상태: **아카이브 (동결)** — v1 프로토타입 완성(M1~M4)과 함께 요구사항·결정 로그(D1~D16)를 동결한 개발 기록입니다. 이후의 설계 결정은 [decisions/](decisions/)에 D17부터 개별 문서(ADR)로, 작업 기록은 [milestones/](milestones/)에 이어서 관리합니다. 이 파일은 더 이상 수정하지 않습니다.
